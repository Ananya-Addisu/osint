...
# ASCII-àðò ïðèâåòñòâèÿ äëÿ ïðîãè
ascii_banner = pyfiglet.figlet_format("@sftclu")
colored_banner = colored(ascii_banner, color='red')  # Êðàñèì â öâåò
print('разработано (@долбоёб)')
print('по всем вопросам к @долбоёб')
print("Взломано by @sftclu")
print('-------------------------------------------------------------')
...
def send_complaint(username, telegram_id, number, email, repeats, complaint_choice, proxies=None):
	...
	if complaint_choice == "1":
		...
		text = f'Добрый день поддержка Telegram! Аккаунт {username}, {telegram_id} использует виртуальный номер купленный на сайте по активации номеров. Отношения к номеру он не имеет, номер никак к нему не относится. Прошу разобраться с этим. Заранее спасибо!'
	...
	try:
		...
		for _ in range(int(repeats)):
			...
			if response.status_code == 200:
				...
				print("ЭТОТ ГОВНОСОФТ НЕ ОТПРАВИЛ РЕПОРТ:. code:", response.status_code)
...
def complaint():
	...
	print(colored("[3] Оскорбление (для терпил)","red"))
	print(colored("[4] Нарушение правил", "magenta"))
	print(colored("[5] Сессии", "red"))
	print(colored("[6] Спам", "magenta"))
	print(colored("[7] Прайс", "red"))
	print(colored("[8] Накрутка", "magenta"))
	complaint_choice = input(colored("[LEAKED BY ENERITY] Введите пункт для этой хуетени: ", "blue"))
	if complaint_choice in ["1", "2", "3", "4", "6", "7", "8"]:

		username = input("[LEAKED BY ENERITY] Введите @username: ")

		telegram_id = input("[LEAKED BY ENERITY] Введите Telegram ID: ")

		repeats = int(input("[LEAKED BY ENERITY] Введите количество жалоб: "))
		...
		username = input("[LEAKED BY ENERITY] Введите юзернейм: ")

		telegram_id = input("[LEAKED BY ENERITY] Введите Telegram ID: ")

		repeats = int(input("[LEAKED BY ENERITY] Введите кол-во жалоб: "))

		number = input("В[LEAKED BY ENERITY] Введите номер телефона аккаунта: ")
		...
		print("Иди нахуй, ты что то не так выбрал.")
complaint()